...
<template>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody>
                    <template v-for="(product, productIdx) in order.products" :key="product.id">
                      <OrderProductListRow :customer-product="product" :idx="productIdx" :refresh-orders="refreshOrders" />
                    <!-- {{ product.name }} -->
                    </template>
                  </tbody>
                </table>
  ...
  <div v-else>
    <UContainer class="flex flex-1 flex-col items-center justify-center">
